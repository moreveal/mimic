...
Следующая реализация должна исправлять семейство SVGGraphicsElement/SVGGElement
и наблюдения bounding box, а не добавлять пустую функцию getBBox. До изменения
кода нужно расширить нативный эталон: пустые группы, примитивы, объединение
дочерних границ, вложенные transforms, detached/display-состояния и receiver из
другого realm. Геометрические наблюдения вычислять без графического backend;
неподдерживаемые формы явно отделить от реализованных.
