...
const w=f.contentWindow,d=w.document,obj=w.eval('({value:17})');d.savedValue=23;
await new Promise(resolve=>{f.onload=resolve;f.src='about:blank?next_document';});
return {status:'ok',value:await (({windowSame:w===f.contentWindow,documentReplaced:d!==f.contentDocument,oldDocumentValue:attempt(()=>d.savedValue),newDocumentValue:attempt(()=>f.contentDocument.savedValue),oldObjectValue:attempt(()=>obj.value)}))};
