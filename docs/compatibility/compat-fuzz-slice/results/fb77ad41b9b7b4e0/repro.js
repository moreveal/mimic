...
try {
const frame=document.createElement('iframe');frame.src='http://localhost:54524/';
await new Promise((resolve,reject)=>{frame.onload=resolve;frame.onerror=()=>reject(new Error('iframe load failed'));document.body.appendChild(frame);});
const other=frame.contentWindow;
return {status:'ok',value:await (({document:attempt(()=>other.document),location:attempt(()=>other.location.href),postMessage:attempt(()=>typeof other.postMessage),self:attempt(()=>other===frame.contentWindow),tag:attempt(()=>tag(other))}))};
} catch(e) {return {status:'exception',error:error(e)};}
})();
  for (const key of ["value", "document", "class"]) {
