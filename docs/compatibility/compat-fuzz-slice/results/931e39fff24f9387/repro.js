...
try {
const item=document.createElement('form');item.name='compat_named';document.body.appendChild(item);
return {status:'ok',value:await (({windowNamed:window.compat_named===item,documentNamed:document.compat_named===item,descriptor:descriptor(window,'compat_named')}))};
} catch(e) {return {status:'exception',error:error(e)};}
})();
  for (const key of ["value", "descriptor"]) {
