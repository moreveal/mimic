...
try {
const live=document.getElementsByTagName('span');const fixed=document.querySelectorAll('span');const liveBefore=live.length;const staticBefore=fixed.length;
return {status:'ok',value:await (({liveBefore,staticBefore,liveAfter:live.length,staticAfter:fixed.length,identity:live===document.getElementsByTagName('span')}))};
} catch(e) {return {status:'exception',error:error(e)};}
})();
  for (const key of ["value", "identity"]) {
