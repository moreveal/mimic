...
const saved=other;const savedDocument=other.document;
await new Promise((resolve,reject)=>{frame.onload=resolve;frame.onerror=()=>reject(new Error('iframe navigation failed'));frame.src='about:blank?compat_navigation';});
return {status:'ok',value:await (({proxy:saved===frame.contentWindow,documentChanged:savedDocument!==frame.contentDocument,parent:frame.contentWindow.parent===window}))};
} catch(e) {return {status:'exception',error:error(e)};}
})();
  for (const key of ["value", "documentChanged"]) {
