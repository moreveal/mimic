...
try {
const item=document.createElement('form');item.id='compat_named';document.body.appendChild(item);
const nodes=document.forms;
return {status:'ok',value:await (({collection:inspect(nodes),index:nodes[0]===item,named:nodes.compat_named===item,namedItem:nodes.namedItem('compat_named')===item,missing:attempt(()=>nodes.item(9)),listIndex:document.querySelectorAll('form')[0]===item,listMissing:attempt(()=>document.querySelectorAll('form').item(9))}))};
} catch(e) {return {status:'exception',error:error(e)};}
})();
  for (const key of ["value", "collection", "ownKeys"]) {
