...
def extended(raw: str | None) -> dict[str, str | bool | list[str]]:
    if not raw:
        return {}
    body = raw.strip()[1:-1]
...
def normalize_idl(sources: dict[str, str], target: dict) -> dict:
    ...
    for partial in partials:
        ...
        if candidates:
            ...
            base["extended"].update(partial["extended"])
        else:
...
(function(host){{
  ...
  for(const spec of catalog){{
    ...
    if(generated){{
      ctor={{[spec.name]:function(){{missing(spec.name,'constructor');throw new TypeError('Illegal constructor')}}}}[spec.name];
    ...
    for(const member of spec.members){{
      ...
      if(member.kind==='operation')Object.defineProperty(target,member.name,{{value:function(){{return missing(spec.name,member.name)}},writable:true,enumerable:true,configurable:true}});
      else if(member.kind==='constant')Object.defineProperty(target,member.name,{{value:Number(member.value)||0,enumerable:true}});
      else Object.defineProperty(target,member.name,{{get:function(){{return missing(spec.name,member.name)}},set:member.readonly?undefined:function(){{return missing(spec.name,member.name)}},enumerable:true,configurable:true}});
    }}
  }}
