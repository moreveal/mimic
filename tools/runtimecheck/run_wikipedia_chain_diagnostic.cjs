...
async function main() {
  ...
  const server = start(
    process.env.MIMIC_CHAIN_BINARY || path.join(build, 'mimic-chain-trace.exe'),
    ...
    path.join(build, 'wiki-chain-server.log'),
  );
  try {
    ...
    for (const mode of ['cold', 'warm']) {
      const workload = start(
        ...
        ['tools/runtimecheck/playwright_wikipedia_profile.js'],
        {
          ...
          MIMIC_PROFILE_OUTPUT: path.join(build, `wiki-chain-${mode}.json`),
        },
        path.join(build, `wiki-chain-${mode}.log`),
      );
      let timedOut = false;
