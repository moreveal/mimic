...
def generate(path):
    ...
    sections=['# Mimic V8 and Chrome 152: Windows x64 baseline',
      'This is '+('a smoke run, insufficient for conclusions.' if m['arguments']['smoke'] else 'a measurement after semantic fixes. No runtime performance optimizations were performed.')+" The original version's correctness check is saved separately in `pre-fix/raw.json`.",
    ...
    sections+=claims+['7. Invalid comparisons after fixes: '+(', '.join(k for k,v in d['gates'].items() if v!='VALID') or 'none at the correctness gate; individual iteration errors remain in raw.json')+'. Before fixes: DOM, async, React, WebAssembly (see pre-fix).',
