"""Build the small Windows GPU/font recipe catalog from paired fingerprint records.
...
def build(source_dir, repo_dir):
    ...
    recipes = []
    for recipe_id, filename in SOURCES.items():
        raw = (source_dir / filename).read_bytes()
        source = json.loads(raw)
        match = re.search(r"Chrome/(\d+)", source["ua"])
        if not match or int(match.group(1)) < 140 or "Microsoft Windows" not in source["tags"]:
            raise ValueError(f"unsupported source record {filename}")
        gl = source["webgl_properties"]
        if "Direct3D11" not in gl["unmaskedRenderer"]:
            raise ValueError(f"non-D3D11 source record {filename}")
