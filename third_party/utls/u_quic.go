...
func UQUICClient(config *QUICConfig, clientHelloID ClientHelloID) *UQUICConn {
	q := newUQUICConn(UClient(nil, config.TLSConfig, clientHelloID, false, false, false))
	q.conn.quic.enableSessionEvents = config.EnableSessionEvents
	return q
}
...
func (q *UQUICConn) SetTransportParameters(params []byte) {
	...
	// The live transport is authoritative, including connection-specific IDs.
