...
// Path verifies and materializes the embedded native library in the user's cache.
...
// Materialize verifies and extracts the embedded native library below cacheRoot.
func Materialize(cacheRoot string) (string, error) {
	...
	// Serialize extraction/repair across processes, not engine execution.
	unlock, err := lockCache(filepath.Join(dir, ".materialize.lock"))
	if err != nil {
		return "", fmt.Errorf("lock shim cache: %w", err)
	}
	defer unlock()
...
// Verify checks that path contains the exact packaged native library.
