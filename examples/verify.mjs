...
let log = '';
try {
  runtime = spawn(resolve(binary), ['--listen', '127.0.0.1:0', '--profile', resolve(here, 'profile.json')], {
    cwd: cache,
    env: { ...process.env, XDG_CACHE_HOME: cache, LOCALAPPDATA: cache, GOV8_SHIM_LIBRARY: '', GOV8_SHIM_DLL: '' },
  ...
  const endpoint = await new Promise((resolveReady, reject) => {
    ...
    runtime.once('exit', code => { clearTimeout(timer); reject(new Error(`Runtime exited ${code}: ${log}`)); });
