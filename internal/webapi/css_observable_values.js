...
const parseCSSTransform=(value,serialize=cssSerializeNumber)=>{
 ...
 let index=0;
 while(index<value.length){while(/\s/.test(value[index]||'')&&index<value.length)index++;if(index===value.length)break;
  const match=/^([a-zA-Z0-9]+)\(/.exec(value.slice(index));if(!match)return null;const key=match[1].toLowerCase(),spec=functions[key];if(!spec)return null;
...
const parseCSSShadow=(value,text=false)=>{
 if(value==='none')return value;const layers=cssSplitTopLevel(value,',');if(!layers)return null;const out=[];
 for(const layer of layers){const tokens=cssValueTokens(layer.replace(/(?<=[\d.)])#/g,' #'));if(!tokens)return null;let color=null,inset=false;const lengths=[];
  for(const token of tokens){const c=cssColorValue(token),length=cssSignedLength(token);if(c!==null){if(color!==null)return null;color=c}else if(token==='inset'&&!text){if(inset)return null;inset=true}else if(length!==null&&!length.endsWith('%'))lengths.push(length);else return null}
  if(lengths.length<2||lengths.length>(text?3:4)||lengths.length>=3&&Number.parseFloat(lengths[2])<0)return null;
...
cssLonghandParsers.set('transform-origin',value=>{const words=cssValueTokens(value);if(!words||words.length<1||words.length>3)return null;const converted=words.map(v=>['left','right','top','bottom','center'].includes(v)?v:cssSignedLength(v));if(converted.includes(null))return null;if(converted.length===1)return ['top','bottom'].includes(converted[0])?'center '+converted[0]:converted[0]+' center';return converted.join(' ')});
...
cssShorthandParsers.set('inset',value=>cssFourValues(value,v=>v==='auto'?v:cssSignedLength(v)));
...
for(const name of ['gap','border-spacing'])cssLonghandParsers.set(name,value=>{const words=cssValueTokens(value);if(!words||words.length<1||words.length>2)return null;const converted=words.map(v=>name==='gap'&&v==='normal'?v:cssLengthValue(v));return converted.includes(null)?null:converted.length===2&&converted[0]===converted[1]?converted[0]:converted.join(' ')});
