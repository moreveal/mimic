...
const consoleObject = (() => {
  ...
  const parseInteger = parseInt,
    parseFloating = parseFloat,
    dateTime = Date.prototype.getTime;
  const regexpSource = Object.getOwnPropertyDescriptor(RegExp.prototype, 'source').get;
  const ErrorClass = Error;
  const nativeKind = host.consoleValueKind,
    proxy = host.cloneIsProxy || host.historyCloneIsProxy;
  const classify = (value) => {
    ...
    if (reference && host.frameConsoleKind)
      return host.frameConsoleKind(reference.frame, reference.handle, reference.realm);
    if (nativeKind) return nativeKind(value);
    if (proxy && proxy(value)) return '';
    if (typeof value === 'function') return 'function';
    try {
      apply(dateTime, value, []);
      return 'date';
    } catch {}
    try {
      apply(regexpSource, value, []);
      return 'regexp';
    } catch {}
    // Non-V8 fallback; native V8 classification is the frozen-Chrome authority.
    try {
      if (value instanceof ErrorClass) return 'error';
    } catch {}
    return '';
  };
    ...
    if (format && args.length > 1 && typeof args[0] === 'string') {
      ...
      for (let i = 0; i < args[0].length - 1 && argument < args.length; i++) {
        ...
        if (specifier === 's') args[argument] = string(v);
          ...
          args[argument] =
            typeof v === 'symbol' ? NaN : specifier === 'f' ? parseFloating(v) : parseInteger(v);
        argument++;
      }
    }
    return args;
  ...
  const emit = (kind, input, format = true) => {
    ...
    const values = argumentsFor(input, format),
      text = [];
    for (let i = 0; i < values.length; i++) {
      ...
      if (v !== null && (type === 'object' || type === 'function')) {
        ...
        if (classify(v))
          try {
            text[i] = string(v);
          } catch {} // Chrome suppresses message-text conversion failures.
  ...
  const count = (value, reset) => {
    ...
    if (reset) {
      if (!apply(del, counts, [key]))
        emit('warning', ["Count for '" + key + "' does not exist"], false);
    } else {
      const n = (apply(get, counts, [key]) || 0) + 1;
      apply(set, counts, [key, n]);
      emit('count', [key + ': ' + n], false);
    }
  ...
  const methods = {
    ...
    trace(...a) {
      emit('trace', a.length ? a : ['console.trace']);
    },
    group(...a) {
      emit('startGroup', a.length ? a : ['console.group']);
    },
    groupCollapsed(...a) {
      emit('startGroupCollapsed', a.length ? a : ['console.groupCollapsed']);
    },
    groupEnd(...a) {
      emit('endGroup', a.length ? a : ['console.groupEnd'], false);
    },
    count(...a) {
      count(a[0], false);
    },
    ...
    assert(...a) {
      if (!a[0]) emit('assert', a.length > 1 ? apply(slice, a, [1]) : ['console.assert']);
    },
    clear() {
      emit('clear', ['console.clear'], false);
    },
    time(...a) {
      timer(a[0], 'start', []);
    },
    timeLog(...a) {
      timer(a[0], 'log', apply(slice, a, [1]));
    },
    timeEnd(...a) {
      timer(a[0], 'end', []);
    },
  ...
  for (const [name, method] of Object.entries(methods)) {
    const value = (...args) => {
      if (host.executionContextActive && !host.executionContextActive()) return;
      return apply(method, undefined, args);
    };
