const compatibilityElementState={};
  // Realm-local semantic state. Canonical nodes remain owned by the DOM host.
  {
    ...
    let documentRootID=host.documentRootID();const childLists=new WeakMap();bootstrapRestoreHooks.push(()=>{documentRootID=host.documentRootID()});
    ...
    accessor(Node.prototype,'childNodes',function(){let list=childLists.get(this);if(!list){const node=this;list=nodeListView(()=>childCount(node),index=>childAt(node,index));childLists.set(this,list)}return list});
    ...
    Object.defineProperty(Document.prototype,'getElementsByName',{value:function(name){return nodeList(compatibilitySelectors.query(this,'[name="'+String(name).replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'"]',false,true))},writable:true,enumerable:true,configurable:true});
    ...
    for(const ctor of [Document,Element])Object.defineProperty(ctor.prototype,'getElementsByClassName',{value:function(names){
      ...
      return cachedHTMLCollection(root,'class',names,()=>tokens.length?compatibilitySelectors.query(root,'*').filter(node=>{const classes=normalize(node.getAttribute('class')||'').split(/[\t\n\f\r ]+/);return tokens.every(token=>classes.includes(token))}).map(node=>host.nodeData(elementSlot(node).nodeId)):[]);
