...
const cssBoxModel=(()=>{
 ...
 const taffyBox=element=>{
  let root=null;for(let node=element;node;node=geometryParent(node)){if(/^(?:inline-)?(?:flex|grid)$/.test(state(node).display))root=node;else if(root)break}if(!root)return null;
  let cache=styleReadCache.taffyLayouts;if(!cache)cache=styleReadCache.taffyLayouts=new WeakMap();let boxes=cache.get(root);
  if(!boxes){const rootLegacy=rect(root),rootState=state(root),rootBasis=containingWidth(root),rootEdges=rootState.edges(rootBasis);let rootWidth=rootState.length(rootState.get('width'),rootBasis);if(rootWidth===null)rootWidth=Math.max(0,rootBasis-rootEdges.mleft-rootEdges.mright);else if(rootState.get('box-sizing')!=='border-box')rootWidth+=rootEdges.pleft+rootEdges.pright+rootEdges.bleft+rootEdges.bright;const rootMin=rootState.length(rootState.get('min-width'),rootBasis),rootMax=rootState.length(rootState.get('max-width'),rootBasis);if(rootMin!==null)rootWidth=Math.max(rootWidth,rootMin);if(rootMax!==null)rootWidth=Math.min(rootWidth,rootMax);const nodes=[],tracks=[],elements=new Map();
   const length=(s,value,auto=true)=>{if(value==null||value===''||['auto','none','normal'].includes(value))return {kind:auto?0:1,value:0};const percent=/^([+-]?[\d.]+)%$/.exec(value);if(percent)return {kind:2,value:Number(percent[1])/100};const resolved=s.length(value,0);return resolved===null?{kind:auto?0:1,value:0}:{kind:1,value:resolved}};
   const edge=(s,prefix,allowAuto=false)=>{const one=side=>{const property=prefix==='border-'?prefix+side+'-width':prefix+side,value=s.get(property);return allowAuto&&value==='auto'?{kind:0,value:0}:length(s,value,false)};return {Top:one('top'),Right:one('right'),Bottom:one('bottom'),Left:one('left')}};
   const align=value=>({start:1,end:2,'flex-start':3,'flex-end':4,center:5,baseline:6,stretch:7,'space-between':8,'space-around':9,'space-evenly':10}[value]||0);
   const trackList=(s,value)=>{const offset=tracks.length,append=token=>{let match=/^repeat\(\s*([1-9]\d*)\s*,\s*(.+)\)$/i.exec(token);if(match){const repeated=cssValueTokens(match[2])||[],count=Math.min(1024,Number(match[1]));for(let i=0;i<count;i++)for(const item of repeated)append(item);return}match=/^([+-]?[\d.]+)fr$/i.exec(token);if(match)tracks.push({kind:3,value:Number(match[1])});else if(token==='min-content')tracks.push({kind:4,value:0});else if(token==='max-content')tracks.push({kind:5,value:0});else if(token==='auto')tracks.push({kind:0,value:0});else {const item=length(s,token);if(item.kind)tracks.push(item)}};for(const token of cssValueTokens(value||'')||[])append(token);return [offset,tracks.length-offset]};
   const placement=value=>{if(!value||value==='auto')return 0;const span=/^span\s+([1-9]\d*)$/.exec(value);if(span)return -Number(span[1]);return /^-?[1-9]\d*$/.test(value)?Number(value):0};
   const append=(node,parent)=>{const s=state(node),index=nodes.length,canLayoutChildren=/^(?:block|(?:inline-)?flex|(?:inline-)?grid)$/.test(s.display),list=canLayoutChildren?children(node).filter(child=>elementSlot(child)?.type==='element'&&rendered(child)):[],own=controlSize(node),text=textOf(node),font=textInfo(node,text),measure=own?[own.width,own.height]:list.length?[-1,-1]:[text?font.width:0,text?font.height:0],parentNode=geometryParent(node),parentIsFlex=parentNode&&/^(?:inline-)?flex$/.test(state(parentNode).display),minWidth=parentIsFlex&&(!s.get('min-width')||s.get('min-width')==='auto')?{kind:1,value:0}:length(s,s.get('min-width')),columns=trackList(s,s.get('grid-template-columns')),rows=trackList(s,s.get('grid-template-rows'));nodes.push({ID:elementSlot(node).nodeId,Parent:parent,Display:s.display==='none'?0:/^(?:inline-)?flex$/.test(s.display)?2:/^(?:inline-)?grid$/.test(s.display)?3:1,Position:['absolute','fixed'].includes(s.position)?1:0,BoxSizing:s.get('box-sizing')==='content-box'?1:0,FlexDirection:{column:1,'row-reverse':2,'column-reverse':3}[s.get('flex-direction')]||0,FlexWrap:{wrap:1,'wrap-reverse':2}[s.get('flex-wrap')]||0,AlignItems:align(s.get('align-items'))||7,AlignSelf:align(s.get('align-self')),AlignContent:align(s.get('align-content')),JustifyContent:align(s.get('justify-content')),JustifySelf:align(s.get('justify-self')),JustifyItems:align(s.get('justify-items'))||7,Width:node===root?{kind:1,value:rootWidth}:length(s,s.get('width')),Height:length(s,s.get('height')),MinWidth:minWidth,MinHeight:length(s,s.get('min-height')),MaxWidth:length(s,s.get('max-width')),MaxHeight:length(s,s.get('max-height')),FlexBasis:length(s,s.get('flex-basis')),FlexGrow:Number(s.get('flex-grow'))||0,FlexShrink:Number.isFinite(Number(s.get('flex-shrink')))?Number(s.get('flex-shrink')):1,Margin:edge(s,'margin-',true),Padding:edge(s,'padding-'),Border:edge(s,'border-'),Inset:{Top:length(s,s.get('top'),true),Right:length(s,s.get('right'),true),Bottom:length(s,s.get('bottom'),true),Left:length(s,s.get('left'),true)},GapX:length(s,s.get('column-gap'),false),GapY:length(s,s.get('row-gap'),false),MeasureWidth:measure[0],MeasureHeight:measure[1],GridColumnOffset:columns[0],GridColumnCount:columns[1],GridRowOffset:rows[0],GridRowCount:rows[1],GridColumnStart:placement(s.get('grid-column-start')),GridColumnEnd:placement(s.get('grid-column-end')),GridRowStart:placement(s.get('grid-row-start')),GridRowEnd:placement(s.get('grid-row-end'))});for(const child of list)append(child,index)};
   append(root,-1);for(let i=0;i<nodes.length;i++)elements.set(nodes[i].ID,i===0?root:wrap(nodes[i].ID));const response=JSON.parse(host.layoutTaffy(JSON.stringify({viewport:[rootWidth,host.viewport().height],nodes,tracks})));if(response.error)throw new Error(response.error);boxes=new Map();
   for(const box of response.boxes){const node=elements.get(box.id),edges=state(node).edges(box.width),x=box.x+rootLegacy.x,y=box.y+rootLegacy.y;boxes.set(box.id,{...box,x,y,left:x,top:y,right:x+box.width,bottom:y+box.height,clientWidth:Math.max(0,box.width-edges.bleft-edges.bright),clientHeight:Math.max(0,box.height-edges.btop-edges.bbottom),offsetLeft:x,offsetTop:y})}
   for(const [id,value] of boxes){const node=elements.get(id);let offsetParent=null;if(state(node).position!=='fixed')for(let p=geometryParent(node);p;p=geometryParent(p))if(state(p).position!=='static'||['BODY','TD','TH','TABLE'].includes(tag(p))){offsetParent=p;break}if(offsetParent&&!(tag(offsetParent)==='BODY'&&state(offsetParent).position==='static')){const origin=boxes.get(elementSlot(offsetParent).nodeId)||rect(offsetParent),edges=state(offsetParent).edges(origin.width);value.offsetLeft-=origin.x+edges.bleft;value.offsetTop-=origin.y+edges.btop}}
 ...
 const observedWidthBox=element=>{const box=taffyBox(element);return box?{width:box.width,clientWidth:box.clientWidth,edges:state(element).edges(box.width)}:widthBox(element)};
 const observedHeightBox=element=>{const box=taffyBox(element);return box?{height:box.height,clientHeight:box.clientHeight,edges:state(element).edges(box.width)}:heightBox(element)};
 return {width:observedWidth,rect:observedRect,state,size,widthBox:observedWidthBox,heightBox:observedHeightBox,hasBox,fixedContainer};
})();
