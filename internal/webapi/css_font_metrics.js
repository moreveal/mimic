...
const cssFontParent=n=>{const parent=cssObservationParent(n);return shadowSlots.get(parent)?.host||parent||shadowSlots.get(n)?.host||null};
...
const cssAbsoluteLengthScales={px:1,pt:96/72,pc:16,in:96,cm:96/2.54,mm:96/25.4,q:96/101.6};
const cssResolveLength=(input,context)=>{
 ...
 if(term){const scale=term.unit==='em'?context.em:term.unit==='rem'?context.rem:term.unit==='%'?context.percent/100:cssAbsoluteLengthScales[term.unit];return Number.isFinite(scale)?term.number*scale:null}
 ...
 if(product){
  const leftNumber=cssNumberRegex.test(product[1])?Number(product[1]):null,rightNumber=cssNumberRegex.test(product[3])?Number(product[3]):null;
  if(product[2]==='*'){
   if(leftNumber!==null){const right=cssResolveLength(product[3],context);return right===null?null:leftNumber*right}
   if(rightNumber!==null){const left=cssResolveLength(product[1],context);return left===null?null:left*rightNumber}
  }else if(rightNumber!==null&&rightNumber!==0){const left=cssResolveLength(product[1],context);return left===null?null:left/rightNumber}
  return null;
 }
...
const cssComputedRootFontSize=()=>{
 if(styleReadCache&&Object.hasOwn(styleReadCache,'rootFontSize'))return styleReadCache.rootFontSize;
 ...
 const root=wrap(host.firstElementChild(realmDocumentRootID)),size=root?(cssComputedFontSize(root)??16):16;
 if(styleReadCache)styleReadCache.rootFontSize=size;
 return size;
};
const cssGeometryLengthContext=(element,percent=0,em)=>({
 get em(){return em??cssComputedFontSize(element)??16},
 get rem(){return cssComputedRootFontSize()},percent
});
const cssComputedFontSize=element=>{
 const cache=styleReadCache&&(styleReadCache.fontSizes||(styleReadCache.fontSizes=new WeakMap()));
 ...
 for(let i=chain.length-1;i>=0;i--){const n=chain[i];if(cache?.has(n)){size=cache.get(n);if(i===chain.length-1)rootSize=size;continue}const entries=computedCSSDeclarations(n);let raw=entries.find(e=>e.name==='font-size')?.value;const source=raw==null?'presentation-or-inherited':'css';
 ...
 cache?.set(element,size);return size;
};
