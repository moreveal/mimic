...
registerBootstrapCallback('installDevPreview', (kind) =>
  kind === 'version'
    ? host.observationVersion() +
      ...
      ':' +
      devPreviewFormRevision +
      ':' +
      compatibilityScrolling.revision()
    : {
        width: host.viewport().width,
        height: host.viewport().height,
        windowScroll: [windowScrollX, windowScrollY],
        scroll: compatibilityScrolling.snapshot(),
        modals: compatibilityElementState.modalNodes(),
        styles: constructedStyleSheets.previewSources(document),
      },
);
