...
const glslObservations=(()=>{
 ...
 const types=new Set(['float','int','bool','vec2','vec3','vec4','ivec2','ivec3','ivec4','sampler2D']);
 ...
 function compile(source,shaderType){
  ...
  const knownCall=name=>types.has(name)||Object.hasOwn(builtins,name)||['min','max','pow','mod','clamp','mix','step','smoothstep','dot','length','normalize','texture','texture2D'].includes(name);
  ...
  function infer(n,scope){switch(n.op){
   ...
   case 'call':{const args=n.args.map(a=>infer(a,scope));if(['texture','texture2D'].includes(n.name)){if(args.length!==2||args[0]!=='sampler2D'||args[1]!=='vec2')invalid('Texture arguments');return 'vec4'}if(types.has(n.name)){if(!args.length)invalid('Empty constructor');const size=width(n.name),total=args.reduce((sum,a)=>sum+width(a),0);if(size===1&&args.length!==1||size>1&&total!==1&&(total<size||total-width(args[args.length-1])>=size))invalid('Constructor components');return n.name}const count=['clamp','mix','smoothstep'].includes(n.name)?3:['min','max','pow','mod','step','dot'].includes(n.name)?2:1;if(args.length!==count)invalid('Builtin argument count');return ['length','dot'].includes(n.name)?'float':args.reduce((a,b)=>width(a)>=width(b)?a:b)}
  }}
