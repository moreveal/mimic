  ...
  const nativeFunctionRecordGet=WeakMap.prototype.get.bind(nativeFunctionNames),nativeFunctionNameSet=WeakMap.prototype.set.bind(nativeFunctionNames),functionSourceApply=Reflect.apply;
  const nativeFunctionNameGet=fn=>nativeFunctionRecordGet(fn)?.name;
  const nativeFunctionSourceGet=fn=>{const record=nativeFunctionRecordGet(fn);return record===undefined?undefined:record.source===undefined?'function '+record.name+'() { [native code] }':record.source};
  const markForeignFunctionSource=(fn,source)=>nativeFunctionNameSet(fn,{__proto__:null,source});
  ...
  Function.prototype.toString=({toString(){const source=nativeFunctionSourceGet(this);return source===undefined?functionSourceApply(engineFunctionToString,this,[]):source}}).toString;
  const nativeFunctionSourceState=[nativeFunctionSourceGet,engineFunctionToString];
  const markNative=(fn,name,prefix='')=>{if(typeof fn==='function')nativeFunctionNameSet(fn,{__proto__:null,name:prefix?prefix+String(name):String(name)})};
