...
function checkElementVisibility(options={}){
    ...
    return withStyleReadCache(()=>{
      const flags={visibilityProperty:!!(options.visibilityProperty||options.checkVisibilityCSS),opacityProperty:!!(options.opacityProperty||options.checkOpacity),contentVisibilityAuto:!!options.contentVisibilityAuto};
      ...
      if((options.visibilityProperty||options.checkVisibilityCSS)&&['hidden','collapse'].includes(getComputedStyle(this).visibility))return false;
      for(let element=this;element;element=geometryParent(element)){
        ...
        if((options.opacityProperty||options.checkOpacity)&&Number(state.get('opacity')??1)===0)return false;
      }
      return true;
    });
}
Object.defineProperty(Element.prototype,'checkVisibility',{
  value:checkElementVisibility,writable:true,enumerable:true,configurable:true
});
