...
const trustedNativeString = String,
  trustedObjectCreate = Object.create;
const trustedRegExpTest = Function.prototype.call.bind(RegExp.prototype.test);
const trustedString = (value) => {
  if (typeof value === 'symbol') throw new TypeError('Cannot convert a Symbol value to a string');
  return trustedNativeString(value);
};
const trustedIncludes = Function.prototype.call.bind(Array.prototype.includes);
const trustedSetHas = Function.prototype.call.bind(Set.prototype.has),
  trustedSetAdd = Function.prototype.call.bind(Set.prototype.add);
...
const trustedTypeOf = (value) => {
  ...
  return kind === 'TrustedHTML' || kind === 'TrustedScript' || kind === 'TrustedScriptURL'
    ? kind
    : null;
};
const trustedSource = (value, kind) =>
  callRealmBinding(value, requireRealmBinding(value, kind), 'source', []);
...
class TrustedHTML {
  ...
  toString() {
    return trustedSource(this, 'TrustedHTML');
  }
  toJSON() {
    return trustedSource(this, 'TrustedHTML');
  }
...
class TrustedScript {
  ...
  toString() {
    return trustedSource(this, 'TrustedScript');
  }
  toJSON() {
    return trustedSource(this, 'TrustedScript');
  }
...
class TrustedScriptURL {
  ...
  toString() {
    return trustedSource(this, 'TrustedScriptURL');
  }
  toJSON() {
    return trustedSource(this, 'TrustedScriptURL');
  }
...
const trustedPolicySlots = new WeakMap(),
  trustedPolicyGet = WeakMap.prototype.get.bind(trustedPolicySlots);
const trustedFactorySlots = new WeakMap(),
  trustedFactoryGet = WeakMap.prototype.get.bind(trustedFactorySlots);
...
class TrustedTypePolicy {
  ...
  get name() {
    return callRealmBinding(this, requireRealmBinding(this, 'TrustedTypePolicy'), 'name', []);
  }
  createHTML(input, ...args) {
    return trustedPolicyInvoke(this, 'createHTML', input, args, arguments.length);
  }
  createScript(input, ...args) {
    return trustedPolicyInvoke(this, 'createScript', input, args, arguments.length);
  }
  createScriptURL(input, ...args) {
    return trustedPolicyInvoke(this, 'createScriptURL', input, args, arguments.length);
  }
}
let trustedDefaultFactory;
const trustedEventAttributes = new Set(
  host.trustedTypesEventAttributes ? host.trustedTypesEventAttributes() : [],
);
  ...
  for (const name of ['HTMLElement', 'SVGElement', 'MathMLElement', 'Window']) {
    ...
    while (prototype && prototype !== Object.prototype) {
      for (const key of Object.getOwnPropertyNames(prototype))
        if (key.startsWith('on')) trustedEventAttributes.add(key);
...
const trustedAttributeInfo = (tag, name, ns = 'http://www.w3.org/1999/xhtml', attrNS = '') => {
  ...
  const html = ns === 'http://www.w3.org/1999/xhtml',
    svg = ns === 'http://www.w3.org/2000/svg';
  if (
    (html || svg || ns === 'http://www.w3.org/1998/Math/MathML') &&
    !attrNS &&
    trustedSetHas(trustedEventAttributes, name)
  )
    return ['TrustedScript', 'Element ' + name];
  if (html && !attrNS) {
    ...
    if (tag === 'object' && name === 'codebase')
      return ['TrustedScriptURL', 'HTMLObjectElement codeBase'];
  ...
  if (
    ...
    tag === 'script' &&
    name === 'href' &&
    (!attrNS || attrNS === 'http://www.w3.org/1999/xlink')
  )
...
class TrustedTypePolicyFactory {
  constructor(token) {
    ...
    const state = {
      defaultPolicy: null,
      names: new Set(),
      emptyHTML: trustedValue(TrustedHTML, ''),
      emptyScript: trustedValue(TrustedScript, ''),
    };
    trustedFactorySlots.set(this, state);
    trustedDefaultFactory = this;
    registerRealmBinding(this, 'TrustedTypePolicyFactory', {
      emptyHTML: () => state.emptyHTML,
      emptyScript: () => state.emptyScript,
      ...
      createPolicy: (name, options) => {
        ...
        for (const rules of policy.rules) {
          if (
            !trustedRegExpTest(/^[a-zA-Z0-9\-#=_/@.%]*$/, name) ||
            (!trustedIncludes(rules, name) && !trustedIncludes(rules, '*'))
          )
            throw new TypeError(prefix + 'Policy "' + name + '" disallowed.');
          if (trustedSetHas(state.names, name) && !trustedIncludes(rules, "'allow-duplicates'"))
            throw new TypeError(prefix + 'Policy with name "' + name + '" already exists.');
        }
        if (name === 'default' && state.defaultPolicy)
          throw new TypeError(prefix + 'Policy with name "default" already exists.');
        const result = new TrustedTypePolicy(hostToken, name, options);
        trustedSetAdd(state.names, name);
  ...
  createPolicy(name, options = {}) {
    ...
    if (options != null && !['object', 'function'].includes(typeof options))
      throw new TypeError(
        "Failed to execute 'createPolicy' on 'TrustedTypePolicyFactory': The provided value is not of type 'TrustedTypePolicyOptions'.",
      );
    const captured = {};
    for (const method of trustedMethods) {
      const callback = options?.[method];
      if (callback !== undefined && typeof callback !== 'function')
        throw new TypeError(
          "Failed to execute 'createPolicy' on 'TrustedTypePolicyFactory': Failed to read the '" +
            method +
            "' property from 'TrustedTypePolicyOptions': The given value is not a function.",
        );
      captured[method] = callback;
    }
    return callRealmBinding(this, binding, 'createPolicy', [name, captured]);
  }
  isHTML(value) {
    requireRealmBinding(this, 'TrustedTypePolicyFactory');
    if (!arguments.length)
      throw new TypeError(
        "Failed to execute 'isHTML' on 'TrustedTypePolicyFactory': 1 argument required, but only 0 present.",
      );
    return trustedTypeOf(value) === 'TrustedHTML';
  }
  isScript(value) {
    requireRealmBinding(this, 'TrustedTypePolicyFactory');
    if (!arguments.length)
      throw new TypeError(
        "Failed to execute 'isScript' on 'TrustedTypePolicyFactory': 1 argument required, but only 0 present.",
      );
    return trustedTypeOf(value) === 'TrustedScript';
  }
  isScriptURL(value) {
    requireRealmBinding(this, 'TrustedTypePolicyFactory');
    if (!arguments.length)
      throw new TypeError(
        "Failed to execute 'isScriptURL' on 'TrustedTypePolicyFactory': 1 argument required, but only 0 present.",
      );
    return trustedTypeOf(value) === 'TrustedScriptURL';
  }
  get emptyHTML() {
    return callRealmBinding(
      this,
      requireRealmBinding(this, 'TrustedTypePolicyFactory'),
      'emptyHTML',
      [],
  ...
  getAttributeType(tag, name, ns = '', attrNS = '') {
    ...
    tag = trustedString(tag);
    name = trustedString(name);
    ns = ns == null ? '' : trustedString(ns);
    attrNS = attrNS == null ? '' : trustedString(attrNS);
    if (ns && ns !== 'http://www.w3.org/1999/xhtml' && ns !== 'http://www.w3.org/2000/svg')
      return null;
  ...
  getPropertyType(tag, name, ns = '') {
    requireRealmBinding(this, 'TrustedTypePolicyFactory');
    if (arguments.length < 2) throw new TypeError('Not enough arguments');
    tag = trustedString(tag).toLowerCase();
    name = trustedString(name);
    ns = ns == null ? '' : trustedString(ns);
    if (name === 'innerHTML' || name === 'outerHTML') return 'TrustedHTML';
    if (ns === 'http://www.w3.org/2000/svg' && tag === 'script' && name === 'href')
      return 'TrustedScriptURL';
    if (ns && ns !== 'http://www.w3.org/1999/xhtml') return null;
    if (tag === 'script' && ['text', 'textContent', 'innerText'].includes(name))
      return 'TrustedScript';
    if (
      (tag === 'script' && name === 'src') ||
      (tag === 'object' && ['data', 'codeBase'].includes(name)) ||
      (tag === 'embed' && name === 'src')
    )
      return 'TrustedScriptURL';
  ...
  getTypeMapping(ns = '') {
    ...
    if (ns && ns !== 'http://www.w3.org/1999/xhtml') return null;
    const attributes = {};
    for (const name of trustedEventAttributes) attributes[name] = 'TrustedScript';
    return {
      '*': { attributes, properties: { innerHTML: 'TrustedHTML', outerHTML: 'TrustedHTML' } },
      embed: { attributes: { src: 'TrustedScriptURL' }, properties: { src: 'TrustedScriptURL' } },
      iframe: { attributes: { srcdoc: 'TrustedHTML' }, properties: { srcdoc: 'TrustedHTML' } },
      object: {
        attributes: { data: 'TrustedScriptURL', codebase: 'TrustedScriptURL' },
        properties: { data: 'TrustedScriptURL', codeBase: 'TrustedScriptURL' },
      },
      script: {
        attributes: { href: 'TrustedScriptURL', src: 'TrustedScriptURL' },
        properties: {
...
const trustedCodeSink = (source) =>
  trustedStartsWith(source, '(function anonymous') ||
  trustedStartsWith(source, '(function* anonymous') ||
  trustedStartsWith(source, '(async function anonymous') ||
  trustedStartsWith(source, '(async function* anonymous')
    ? 'Function'
    : 'eval';
const trustedEnforceString = (text, kind, sink, prefix, code = false) => {
  const state = host.trustedTypesPolicy();
  if (!state.required) return text;
  const policy = trustedFactoryGet(trustedDefaultFactory)?.defaultPolicy;
  const callback =
    policy &&
    trustedPolicyGet(policy).options[
      kind === 'TrustedHTML'
        ? 'createHTML'
        : kind === 'TrustedScript'
          ? 'createScript'
  ...
  if (callback) {
    ...
    if (result != null) {
      const converted = trustedString(result);
      if (code && converted !== text)
        throw new EvalError(
  ...
  throw new TypeError(
    prefix +
      ...
      kind +
      "' assignment" +
      (policy ? " and the 'default' policy failed to execute." : '.'),
  );
};
const trustedConvert = (value, kind, sink, prefix, owner = null) => {
  ...
  if (owner) {
    const slot = elementSlot(owner);
    if (slot) {
      const remote = host.trustedTypesOwner(slot.nodeId);
      if (remote) return unwrapCrossRealm(remote.frame, remote)(text, kind, sink, prefix);
    }
  }
...
const evalSourceResolver = (value, isCodeLike = false, debuggerUnsafeEval = false) => {
  ...
  try {
    source =
      kind === 'TrustedScript'
        ? trustedSource(value, kind)
        : trustedEnforceString(value, 'TrustedScript', sink, '', true);
  } catch {
    failed = true;
  }
  const blocked = host.trustedTypesPolicy().evalBlocked;
  if (blocked && !debuggerUnsafeEval)
    throw new EvalError(
...
const trustedTimerArgument = (handler) =>
  typeof handler === 'function' || trustedTypeOf(handler) === 'TrustedScript'
    ? handler
    : trustedString(handler);
