...
const constructedStyleSheets = (() => {
  ...
  function makeRule(node,sheet,parent=null) {
    ...
    if(node.type==='Rule')Object.defineProperty(rule,'selectorText',{get:()=>preludeText(node.prelude),set:value=>{try{node.prelude=parse(String(value),{context:'selectorList'})}catch{}},configurable:true});
  ...
  function adoption(root) {
    ...
    if(!value){value=new Proxy([],{set(target,key,sheet){if(typeof key==='string'&&/^\d+$/.test(key)&&!sheets.has(sheet))throw new TypeError('Value is not a constructed CSSStyleSheet');return Reflect.set(target,key,sheet)}});adopted.set(root,value)}
    return value;
  }
