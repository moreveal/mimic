...
// This file runs inside either realm closure after shared primitives and streams_vendor.
{
  ...
  const unusable = record => record.stream !== null && (record.stream.locked || disturbed(record.stream));
  ...
  class FetchHeaders extends BaseHeaders {
    append(name,value){headersState(this,arguments.length,2);name=String(name).toLowerCase();const guard=guards.get(this);if(guard==='immutable')throw new TypeError('Headers are immutable');if(guard==='request'&&forbidden(name)||guard==='response'&&['set-cookie','set-cookie2'].includes(name))return;super.append(name,value)}
    set(name,value){headersState(this,arguments.length,2);name=String(name).toLowerCase();const guard=guards.get(this);if(guard==='immutable')throw new TypeError('Headers are immutable');if(guard==='request'&&forbidden(name)||guard==='response'&&['set-cookie','set-cookie2'].includes(name))return;super.set(name,value)}
    delete(name){headersState(this,arguments.length,1);name=String(name).toLowerCase();const guard=guards.get(this);if(guard==='immutable')throw new TypeError('Headers are immutable');if(guard==='request'&&forbidden(name)||guard==='response'&&['set-cookie','set-cookie2'].includes(name))return;super.delete(name)}
  }
  ...
  async function consume(object){
    const body=slot(bodies,object);if(unusable(body))throw new TypeError('Body is locked or already used');if(body.stream===null)return new Uint8Array();
    const reader=body.stream.getReader(),parts=[];let size=0;
    try{for(;;){const next=await reader.read();if(next.done)break;if(!(next.value instanceof Uint8Array))throw new TypeError('Body stream chunk must be Uint8Array');parts.push(next.value);size+=next.value.byteLength}}
    finally{reader.releaseLock();if(body.cleanup)body.cleanup()}
  ...
  function cloneBody(object){const body=slot(bodies,object);if(unusable(body))throw new TypeError('Body is locked or already used');if(body.stream===null)return {stream:null,type:body.type};const [left,right]=body.stream.tee();body.stream=left;return {stream:right,type:body.type}}
  ...
  function createResponse(record,body){const response=Object.create(FetchResponse.prototype);responses.set(response,record);bodies.set(response,body);return response}
  class FetchResponse {
    constructor(body=null,init={}){
      const status=init.status===undefined?200:(Number(init.status)>>>0)&65535;if(status<200||status>599)throw new RangeError('Invalid response status');
      const statusText=String(init.statusText||'');if(/[^\x09\x20-\x7e\x80-\xff]/.test(statusText))throw new TypeError('Invalid status text');
      if(body!=null&&nullBodyStatus(status))throw new TypeError('Response status cannot have a body');
      const extracted=extract(body),list=headers(init.headers,'response');if(extracted.type&&!list.has('content-type'))list.set('content-type',extracted.type);
      responses.set(this,{status,statusText,headers:list,url:'',type:'default',redirected:false});bodies.set(this,extracted);
    }
  ...
  bodyMethods(FetchResponse.prototype);Object.defineProperty(FetchResponse,'name',{value:'Response'});Object.defineProperty(FetchResponse.prototype,Symbol.toStringTag,{value:'Response',configurable:true});
  const normalizedMethods=new Set(['DELETE','GET','HEAD','OPTIONS','POST','PUT']);
  function dependentSignal(parent){const controller=new globalThis.AbortController();if(parent!=null){if(!(parent instanceof globalThis.AbortSignal))throw new TypeError('signal must be AbortSignal');if(parent.aborted)controller.abort(parent.reason);else parent.addEventListener('abort',()=>controller.abort(parent.reason),{once:true})}return controller.signal}
  class FetchRequest {
    constructor(input,init={}){
      ...
      if(!['omit','same-origin','include'].includes(credentials)||!['default','no-store','reload','no-cache','force-cache','only-if-cached'].includes(cache)||!['follow','error','manual'].includes(redirect))throw new TypeError('Invalid request option');
      if(cache==='only-if-cached'&&mode!=='same-origin')throw new TypeError('only-if-cached requires same-origin');if(mode==='no-cors'&&!['GET','HEAD','POST'].includes(method))throw new TypeError('Invalid no-cors method');
      const supplied=init.body!=null;let body;
      if(supplied)body=extract(init.body);else if(previous){const old=slot(bodies,input);if(unusable(old))throw new TypeError('Request body already used');body={...old}}else body=extract(null);
      if(['GET','HEAD'].includes(method)&&body.stream!==null)throw new TypeError('GET/HEAD request cannot have body');
      if(supplied&&init.body instanceof Streams&&init.duplex!=='half')throw new TypeError('Streaming body requires duplex half');
      if(init.duplex!==undefined&&init.duplex!=='half')throw new TypeError('Invalid duplex');
      const list=headers(init.headers===undefined?previous?.headers:init.headers,'request');if(body.type&&!list.has('content-type'))list.set('content-type',body.type);
      const signal=dependentSignal(init.signal===undefined?previous?.signal:init.signal),referrer=String(init.referrer??previous?.referrer??'about:client');
      if(previous&&!supplied&&body.stream!==null)body.stream=body.stream.pipeThrough(new globalThis.TransformStream());
      requests.set(this,{url:url.href,method,headers:list,mode,credentials,cache,redirect,referrer,referrerPolicy:String(init.referrerPolicy??previous?.referrerPolicy??''),integrity:String(init.integrity??previous?.integrity??''),keepalive:Boolean(init.keepalive??previous?.keepalive??false),signal,destination:'',duplex:'half'});bodies.set(this,body);
    }
    clone(){const record=slot(requests,this),body=cloneBody(this),copy=Object.create(FetchRequest.prototype);requests.set(copy,{...record,headers:headers(record.headers,'request'),signal:dependentSignal(record.signal)});bodies.set(copy,body);return copy}
  }
  for(const name of ['url','method','headers','mode','credentials','cache','redirect','referrer','referrerPolicy','integrity','keepalive','signal','destination','duplex'])Object.defineProperty(FetchRequest.prototype,name,{get(){return slot(requests,this)[name]},configurable:true,enumerable:true});
  bodyMethods(FetchRequest.prototype);Object.defineProperty(FetchRequest,'name',{value:'Request'});Object.defineProperty(FetchRequest.prototype,Symbol.toStringTag,{value:'Request',configurable:true});
  ...
  Object.defineProperty(NativeBlob.prototype,'stream',{value:function(){const blob=this;return new Streams({type:'bytes',async start(controller){try{const value=new Uint8Array(await blob.arrayBuffer());if(value.length)controller.enqueue(value);controller.close()}catch(error){controller.error(error)}}})},writable:true,configurable:true,enumerable:true});
  let nextFetchID=0;
  async function fetch(input,init={}){
    const request=new FetchRequest(input,init),signal=request.signal,id='fetch-'+(++nextFetchID);
    if(signal.aborted)throw signal.reason;
    let abortListener;
    const aborted=new Promise((resolve,reject)=>{abortListener=()=>{if(typeof host.abortFetch==='function')host.abortFetch(id);reject(signal.reason)};signal.addEventListener('abort',abortListener,{once:true})});
    try{
      const operation=(async()=>{const body=await consume(request);if(signal.aborted)throw signal.reason;
        let raw;
        try{raw=await host.fetch(request.url,request.method,Object.fromEntries(request.headers),Array.from(body),id,{credentials:request.credentials,mode:request.mode,cache:request.cache,redirect:request.redirect,referrer:request.referrer})}
        catch(error){if(signal.aborted)throw signal.reason;throw new TypeError('Failed to fetch')}
        if(signal.aborted)throw signal.reason;
