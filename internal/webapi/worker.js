(function(host){
  'use strict';
  const listeners=new Map(),handlers={message:null,messageerror:null,error:null};
  class EventTarget {
    addEventListener(type,listener){if(typeof listener!=='function')return;const key=String(type),values=listeners.get(key)||[];values.push(listener);listeners.set(key,values)}
    removeEventListener(type,listener){const values=listeners.get(String(type))||[],index=values.indexOf(listener);if(index>=0)values.splice(index,1)}
    dispatchEvent(event){for(const listener of (listeners.get(event.type)||[]).slice())listener.call(globalThis,event);const handler=handlers[event.type];if(typeof handler==='function')handler.call(globalThis,event);return true}
  }
  ...
  const trustedValueSlots=new WeakMap(),trustedPolicySlots=new WeakMap(),trustedFactorySlots=new WeakMap(),illegalTrusted=name=>{throw new TypeError('Illegal constructor: '+name)},trustedValue=(Ctor,value)=>{const result=Object.create(Ctor.prototype);trustedValueSlots.set(result,String(value));return result};
  class TrustedHTML { constructor(){illegalTrusted('TrustedHTML')} toString(){return trustedValueSlots.get(this)} toJSON(){return this.toString()} }
  class TrustedScript { constructor(){illegalTrusted('TrustedScript')} toString(){return trustedValueSlots.get(this)} toJSON(){return this.toString()} }
  class TrustedScriptURL { constructor(){illegalTrusted('TrustedScriptURL')} toString(){return trustedValueSlots.get(this)} toJSON(){return this.toString()} }
  ...
  const navigatorData=host.navigator(),locationData=host.location(),workerSubtle=Object.create(SubtleCrypto.prototype);
  ...
  for(const name of ['href','origin','protocol','host','hostname','port','pathname','search','hash'])Object.defineProperty(WorkerLocation.prototype,name,{get(){return locationData[name]},enumerable:true,configurable:true});
