...
(()=>{
 ...
 method('GPUCommandEncoder','finish',(e,descriptor={})=>{if(e.finished){validation(e.device,'Encoder already finished');e.valid=false}e.finished=true;return make('GPUCommandBuffer',{device:e.device,label:String(descriptor.label||''),commands:e.commands.slice(),valid:e.valid,submitted:false})});
 method('GPUQueue','submit',(q,values)=>{const commands=Array.from(values,v=>check(v,'GPUCommandBuffer')),seen=new Set();for(const c of commands){if(c.device!==q.device||!c.valid||c.submitted||seen.has(c)||c.commands.some(op=>!available(op.b,q.device,8)||op.a&&!available(op.a,q.device,4))){validation(q.device,'Invalid submission');return}seen.add(c)}if(q.device.destroyed)return;for(const c of commands){c.submitted=true;for(const op of c.commands)if(op.kind==='copy')op.b.bytes.set(op.a.bytes.subarray(op.sourceOffset,op.sourceOffset+op.size),op.destinationOffset);else op.b.bytes.fill(0,op.destinationOffset,op.destinationOffset+op.size)}});
