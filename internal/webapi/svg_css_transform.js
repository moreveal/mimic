...
const cssTransform=(n,box,raw)=>{
 ...
 const len=(value,axis)=>{const v=cssResolveLength(value,{em:size,rem:rootSize,percent:axis?height:width});if(v===null||!Number.isFinite(v))return fail('unsupported-length');return v};
 ...
 while(rest){if(++count>256)return fail('transform-list-limit');const match=/^([a-zA-Z0-9]+)\(\s*([^()]*)\)\s*/.exec(rest);if(!match)return fail('unsupported-transform-syntax');rest=rest.slice(match[0].length);const name=match[1].toLowerCase(),args=match[2].split(/\s*,\s*|\s+/).filter(Boolean);let m;
  ...
  else if(name==='scale3d'){arity(3);const a=numeric();if(a[2]!==1)return fail('nonunit-scale-z');m=[a[0],0,0,a[1],0,0]}
  ...
  else if(['scale','scalex','scaley'].includes(name)){arity(1,name==='scale'?2:1);const a=numeric();m=[name==='scaley'?1:a[0],0,0,name==='scalex'?1:a[1]??a[0],0,0]}
  else if((name==='rotate'||name==='rotatez')){arity(1);const a=angle(args[0]);m=[Math.cos(a),Math.sin(a),-Math.sin(a),Math.cos(a),0,0]}
