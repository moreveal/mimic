  ...
  (()=>{
    ...
    for(const [kind,type] of [['webgl','WebGLRenderingContext'],['webgl2','WebGL2RenderingContext']]){
      ...
      method('bindBuffer',(s,t,value)=>{t=target(s,t);if(!t)return;if(value===null){s.bindings.set(t,null);return}const r=resource(s,value,'WebGLBuffer');if(!r)return;if(r.target&&((r.target===34963)!==(t===34963))){error(s,1282);return}r.target=t;r.bound=true;s.bindings.set(t,value)});
      method('deleteBuffer',(s,value)=>{if(value===null)return;const r=resources.get(value);if(r?.owner===s&&r.type==='WebGLBuffer'&&r.deleted)return;const own=resource(s,value,'WebGLBuffer');if(!own)return;own.deleted=true;own.data=new Uint8Array(0);for(const [t,b] of s.bindings)if(b===value)s.bindings.set(t,null)});
      const bound=(s,t)=>{t=target(s,t);if(!t)return null;const b=s.bindings.get(t);if(!b){error(s,1282);return null}return resources.get(b)};
      ...
      const storage=s=>{const {width,height}=s.dim;if(width*height>16*1024*1024)fail('drawing buffer allocation limit');if(!s.pixels||s.width!==width||s.height!==height){s.width=width;s.height=height;s.pixels=new Uint8Array(width*height*4);if(!s.attributes.alpha)for(let i=3;i<s.pixels.length;i+=4)s.pixels[i]=255}materializePrograms(s);return s.pixels};
