...
const cssWideValue = (value) =>
  ['initial', 'inherit', 'unset', 'revert', 'revert-layer'].includes(value);
const expandCSSDeclaration = (entry) => {
  const components = cssShorthandComponents[entry.name];
  if (!components) return [entry];
  ...
  if (parser) {
    const values = parser(entry.value);
    return values
      ? (cssOrdinaryShorthandOrder[entry.name] || components).map((name) => ({
          name,
          value: values[components.indexOf(name)],
          priority: entry.priority,
        }))
      : [];
  }
...
const readCSSDeclaration = (entries, name) => {
  ...
  if (selected.some((e) => !e) || selected.some((e) => e.priority !== selected[0].priority))
    return '';
  if (
    selected.every((e) => e.pending?.name === name && e.pending.value === selected[0].pending.value)
  )
    return selected[0].pending.value;
  if (selected.every((e) => e.value === selected[0].value) && cssWideValue(selected[0].value))
    return selected[0].value;
  if (selected.some((e) => e.pending)) return '';
  return serializeOrdinaryCSSShorthand(
    name,
...
const serializeCSSDeclarations = (entries) => {
  ...
  for (const entry of entries) {
    ...
    for (const [candidate, components] of Object.entries(cssShorthandComponents)) {
      ...
      name = candidate;
      value = combined;
      for (const component of components) emitted.add(component);
      break;
    }
    emitted.add(entry.name);
    out.push(name + ': ' + value + (entry.priority ? ' !important' : '') + ';');
  }
  return out.join(' ');
};
