(function(host) {
  ...
  const elementData=new WeakMap(),elementClassLists=new WeakMap(),elementShadows=new WeakMap(),shadowHosts=new Set(),syntheticParents=new WeakMap(),elementSlot=value=>elementData.get(value);let shadowSnapshotRevision=0;
  const setSyntheticParent=(node,parent)=>{syntheticParents.set(node,parent);if(typeof ShadowRoot==='function'&&parent instanceof ShadowRoot)shadowSnapshotRevision++;host.invalidateStyleObservations()};
  const deleteSyntheticParent=node=>{const parent=syntheticParents.get(node),removed=syntheticParents.delete(node);if(removed){if(typeof ShadowRoot==='function'&&parent instanceof ShadowRoot)shadowSnapshotRevision++;host.invalidateStyleObservations()}return removed};
  const setElementShadow=(node,root)=>{elementShadows.set(node,root);shadowSnapshotRevision++;host.invalidateStyleObservations()};
  ...
  class DocumentFragment extends Node {
    ...
    get children(){return cachedHTMLCollection(this,'children','',()=>fragmentState(this).children.map(child=>elementSlot(child)).filter(data=>data?.type==='element'))}
