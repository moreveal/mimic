(function(host) {
  ...
  const parsedStyleRules=text=>{
    ...
    while((match=pattern.exec(source))){
      const selectors=match[1].split(',').map(value=>value.trim()).filter(value=>value&&!value.startsWith('@')&&!value.includes('%')),declarations=parseCSS(match[2]);
      for(const selector of selectors){
        ...
        rules.push({selector,declarations,specificity,pseudo,matches:compatibilitySelectors.compileStyle(pseudo?selector.replace(/::?(before|after)$/,''):selector)});
      }
    }
  ...
  const styleSheetRules=element=>{const root=containingShadowRoot(element)||element.ownerDocument||document,cache=styleReadCache?.rules;if(cache?.has(root))return cache.get(root);const rules=[];let order=0;for(const text of constructedStyleSheets.sources(root))for(const rule of parsedStyleRules(text))rules.push({...rule,order:order++});cache?.set(root,rules);return rules};
  const uncachedCSSDeclarations=(element,pseudo='')=>{const winners=new Map();const accept=(entry,specificity,order)=>{const old=winners.get(entry.name),important=entry.priority==='important';if(!old||Number(important)>Number(old.important)||(important===old.important&&(specificity>old.specificity||(specificity===old.specificity&&order>=old.order))))winners.set(entry.name,{entry,specificity,order,important})};for(const rule of compatibilitySelectors.matchingStyles(element,styleSheetRules(element).filter(rule=>rule.pseudo===pseudo)))for(const entry of rule.declarations)accept(entry,rule.specificity,rule.order);if(!pseudo)for(const entry of inlineCSSDeclarations(element))accept(entry,1000,Number.MAX_SAFE_INTEGER);return Array.from(winners.values(),value=>({...value.entry}))};
  ...
  const layoutWidthFor=element=>{
    ...
    if(width===null){
      const attribute=element.getAttribute('width');width=attribute&&/^\d+(?:\.\d+)?$/.test(attribute)?Number(attribute):0;
  ...
  window.__receiveFrameMessage=(data,origin,sourceId)=>dispatchNative(window,new MessageEvent('message',{data,origin,source:remoteWindow(sourceId)}));globalThis.__receiveFrameMessage=window.__receiveFrameMessage;
  ...
  window.__receiveFrameMessage=(data,origin,sourceId,portIds=[])=>dispatchNative(window,new MessageEvent('message',{data,origin,source:remoteWindow(sourceId),ports:Array.from(portIds,wrapBrowserMessagePort)}));globalThis.__receiveFrameMessage=window.__receiveFrameMessage;
  ...
  def(window,'opener',{get:()=>null,set(value){Object.defineProperty(this,'opener',{value,writable:true,enumerable:true,configurable:true})}});
