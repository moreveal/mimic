...
for (const name of [
  ...
  const normalize = (args) => {
    ...
    if (name === 'DateTimeFormat') {
      ...
      args[1] = new Proxy(args[1] === undefined ? {} : Object(args[1]), {
        get(target, key) {
  ...
  const wrapped = new Proxy(Native, {
    apply(target, self, args) {
      return Reflect.apply(target, self, normalize(args));
    },
    construct(target, args, newTarget) {
      return Reflect.construct(target, normalize(args), newTarget);
    },
  });
  globalThis.Intl[name] = wrapped;
