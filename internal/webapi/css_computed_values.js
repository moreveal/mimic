...
const cssComputedShorthand=(element,name)=>{
 const components=cssComputedShorthands[name],values=components.map(n=>cssComputedValue(element,n)),get=n=>cssComputedValue(element,n),same=values.every(v=>v===values[0]);
 if(values.some(v=>v===''))return '';
 if(['background-position','mask-position','-webkit-mask-position'].includes(name))return values.join(' ');
 if(name==='border'){const sides=['top','right','bottom','left'].map(side=>['width','style','color'].map(part=>get('border-'+side+'-'+part)).join(' '));return sides.every(v=>v===sides[0])?sides[0]:''}
 if(name==='rule'){const row=get('row-rule'),column=get('column-rule');return row===column?row:row+' / '+column}
 if(name==='text-decoration'&&get('text-decoration-line')==='none'&&get('text-decoration-style')==='solid'&&get('text-decoration-thickness')==='auto')return 'none';
 if(name==='-webkit-mask-box-image'&&values.join(' ')==='none 0 fill auto 0 stretch')return 'none';
 if(/^page-break-/.test(name))return values[0]==='page'?'always':values[0];
 if(name==='font')return [get('font-style')==='normal'?'':get('font-style'),get('font-variant-caps')==='normal'?'':get('font-variant-caps'),get('font-weight')==='400'?'':get('font-weight'),get('font-stretch')==='100%'?'':get('font-stretch'),get('font-size')+(get('line-height')==='normal'?'':' / '+get('line-height')),get('font-family')].filter(Boolean).join(' ');
 if(name==='flex-flow')return values.join(' ');
 if(name==='animation'&&get('animation-name')==='none'&&get('animation-duration')==='0s'&&get('animation-delay')==='0s'&&get('animation-timing-function')==='ease'&&get('animation-iteration-count')==='1'&&get('animation-direction')==='normal'&&get('animation-fill-mode')==='none'&&get('animation-play-state')==='running')return 'none';
 if(name==='background')return get('background-color')+' '+get('background-image')+' '+get('background-repeat')+' '+get('background-attachment')+' '+get('background-position')+' / '+get('background-size')+' '+get('background-origin')+' '+get('background-clip');
 if(name==='border-image'&&get('border-image-source')==='none'&&get('border-image-slice')==='100%'&&get('border-image-width')==='1'&&get('border-image-outset')==='0'&&get('border-image-repeat')==='stretch')return 'none';
 if(name==='mask'&&get('mask-image')==='none'&&get('mask-size')==='auto'&&get('mask-repeat')==='repeat'&&get('mask-origin')==='border-box'&&get('mask-clip')==='border-box'&&get('mask-composite')==='add'&&get('mask-mode')==='match-source')return 'none';
 if(name==='border-spacing')return get('-webkit-border-horizontal-spacing')===get('-webkit-border-vertical-spacing')?get('-webkit-border-horizontal-spacing'):get('-webkit-border-horizontal-spacing')+' '+get('-webkit-border-vertical-spacing');
 if(name==='border-block'||name==='border-inline'){const prefix=name+'-start-';if(values.slice(0,3).some((v,i)=>v!==values[i+3]))return '';return get(prefix+'width')+' '+get(prefix+'style')+' '+get(prefix+'color')}
 if(name==='outline')return get('outline-color')+' '+get('outline-style')+' '+get('outline-width');
 if(name==='list-style')return get('list-style-position')+' '+get('list-style-image')+' '+get('list-style-type');
 if(name==='font-synthesis')return ['weight','style','small-caps'].filter(v=>get('font-synthesis-'+v)==='auto').join(' ')||'none';
 if(name==='white-space'){const a=get('white-space-collapse'),b=get('text-wrap-mode');return a==='collapse'?(b==='wrap'?'normal':'nowrap'):a==='preserve'?(b==='wrap'?'pre-wrap':'pre'):a==='preserve-breaks'&&b==='wrap'?'pre-line':values.join(' ')}
 if(name==='grid')return values.join(' / ');
 if(name==='grid-template'&&same)return values[0];
 if(name==='offset')return get('offset-path')+' '+get('offset-distance')+' '+get('offset-rotate')+(get('offset-anchor')==='auto'?'':' / '+get('offset-anchor'));
 if(['container','position-try','text-wrap','text-box','scroll-timeline','view-timeline','timeline-trigger'].includes(name)){
  const kept=values.filter((v,i)=>v!==cssInitialValues.get(components[i]));return kept.length?kept.join(' '):({container:'none','position-try':'none','text-wrap':'wrap','text-box':'normal','scroll-timeline':'none','view-timeline':'none','timeline-trigger':'none'}[name]);
 }
...
const cssComputedValue=(element,name)=>withStyleReadCache(()=>{
 ...
 const sides=vertical?{block:{start:writing.endsWith('-rl')?'right':'left',end:writing.endsWith('-rl')?'left':'right'},inline:{start:rtl?'bottom':'top',end:rtl?'top':'bottom'}}:{block:{start:'top',end:'bottom'},inline:{start:rtl?'right':'left',end:rtl?'left':'right'}};
 const logical={'block-size':vertical?'width':'height','inline-size':vertical?'height':'width','min-block-size':vertical?'min-width':'min-height','min-inline-size':vertical?'min-height':'min-width','max-block-size':vertical?'max-width':'max-height','max-inline-size':vertical?'max-height':'max-width','inset-block-start':sides.block.start,'inset-block-end':sides.block.end,'inset-inline-start':sides.inline.start,'inset-inline-end':sides.inline.end};
 ...
 if(/^(top|right|bottom|left)$/.test(name)&&['fixed','absolute'].includes(state.position)){
  const r=cssBoxModel.rect(element),parent=state.position==='fixed'?null:geometryParent(element),p=parent?cssBoxModel.rect(parent):{x:0,y:0,...host.viewport()};
  return cssSerializeNumber(name==='left'?r.x-p.x:name==='top'?r.y-p.y:name==='right'?p.width-r.x+p.x-r.width:p.height-r.y+p.y-r.height)+'px';
 }
 if(name==='transform-origin'||name==='perspective-origin'){
  if(state.display==='inline'&&!['absolute','fixed'].includes(state.position)&&!replacedGeometryTags.has(elementSlot(element).tagName))return '0px 0px';
  const words=value.split(/\s+/),axis=[box().width,box().height],keywords={left:'0%',top:'0%',center:'50%',right:'100%',bottom:'100%'};
  return words.map((v,i)=>{const n=length(keywords[v]||v,axis[i]||0);return n===null?v:cssSerializeNumber(n)+'px'}).join(' ');
 }
 if(/^border-.*-width$/.test(name)){
  const style=cssComputedValue(element,name.replace(/width$/,'style'));if(style==='none'||style==='hidden')return '0px';
 }
