...
type parser struct {
	tree              *tree
	leadingLFConsumed map[int64]bool
...
func (p *parser) addText(text string) {
	...
	if p.shouldFosterParent() {
		p.fosterParent(p.newNode(NodeData{
			Type: TextNode,
			Data: text,
		}))
		return
...
func (p *parser) addElement(namespaces ...string) {
	namespace := ""
	if len(namespaces) > 0 {
...
func initialIM(p *parser) bool {
	...
	case CommentToken:
		p.doc.AppendChild(p.newNode(NodeData{
			Type: CommentNode,
			Data: p.tok.Data,
		}))
		return true
...
func inBodyIM(p *parser) bool {
	...
	case TextToken:
		...
		case a.Pre, a.Listing:
			if n.FirstChild() == nil && d != "" && !p.leadingLFConsumed[n.id] {
				if p.leadingLFConsumed == nil {
					p.leadingLFConsumed = map[int64]bool{}
				}
				p.leadingLFConsumed[n.id] = true
...
func textIM(p *parser) bool {
	...
	case TextToken:
		d := p.tok.Data
		if n := p.oe.top(); n.DataAtom() == a.Textarea && n.FirstChild() == nil && d != "" && !p.leadingLFConsumed[n.id] {
			if p.leadingLFConsumed == nil {
				p.leadingLFConsumed = map[int64]bool{}
			}
			p.leadingLFConsumed[n.id] = true
...
func afterBodyIM(p *parser) bool {
	...
	case CommentToken:
		...
		p.oe[0].AppendChild(p.newNode(NodeData{
			Type: CommentNode,
			Data: p.tok.Data,
		}))
		return true
