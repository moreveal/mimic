        ...
        loop {
            ...
            match item {
                LineItem::InlineBox(inline_box) => {
                    let box_index = self.line.layout.data.line_items[self.line.data.item_range.start + self.item_index].index;
                    let baseline_offset = self.line.layout.data.inline_box_baseline_offsets.get(box_index).copied().unwrap_or(0.0);
                    let x = self.offset
                    ...
                    return Some(PositionedLayoutItem::InlineBox(PositionedInlineBox {
                        x,
                        y: self.line.data.metrics.baseline - inline_box.height + baseline_offset,
