...
impl<B: Brush> Layout<B> {
    ...
    /// Supply the zero-width CSS parent strut before breaking lines. Values are
    /// in this layout's scaled units. It affects real line metrics, baselines,
    /// inline-box positioning and selection geometry without adding text.
    pub fn set_line_strut(&mut self, ascent: f32, descent: f32, line_height: f32) {
        self.data.line_strut = Some([ascent, descent, line_height]);
    }

    /// Per-box distance from its bottom margin edge to its alignment baseline.
    /// Supplied in the same order as `inline_boxes()`, before line breaking.
    pub fn set_inline_box_baseline_offsets(&mut self, offsets: Vec<f32>) {
        self.data.inline_box_baseline_offsets = offsets;
    }
