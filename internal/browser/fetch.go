...
func fetchRequest(contextID string, target, source *url.URL, args []engine.Value) network.Request {
	request := network.Request{ContextID: contextID, URL: target, Referrer: source, SourceURL: source, Method: strarg(args, 1), Headers: headerMap(arg(args, 2)), Body: byteSlice(arg(args, 3)), Initiator: network.Fetch}
