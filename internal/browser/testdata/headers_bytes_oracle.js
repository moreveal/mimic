(() => {const all=(() => {
  ...
  for(const method of ['append','set']) {
    const log=[];
    capture('conversion.'+method,()=>{new Headers()[method]({toString(){log.push('name');return 'bad name';}},{toString(){log.push('value');return 'ok';}});return 'returned';});
    out['conversionLog.'+method]=log;
    capture('immutable.'+method,()=>{Response.error().headers[method]('x',Symbol());return 'returned';});
  }
  return out;
})()
;return Object.fromEntries(Object.entries(all).filter(([key])=>/^(conversion|conversionLog|immutable)\.|-(name|value)$/.test(key)));})()