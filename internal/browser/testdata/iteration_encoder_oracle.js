(() => {const all=(() => {
 ...
 cap('encoder.destinationShadow',()=>{const a=new Uint8Array(3);Object.defineProperty(a,'length',{value:0});a.set=()=>{throw Error('override')};const result=new TextEncoder().encodeInto('abc',a);return [result,Array.from(a)];});
 cap('encoder.destinationForged',()=>{const a={};Object.setPrototypeOf(a,Uint8Array.prototype);return new TextEncoder().encodeInto('x',a);});
 cap('encoder.destinationClamped',()=>new TextEncoder().encodeInto('x',new Uint8ClampedArray(3)));
 cap('encoder.destinationOffset',()=>{const a=new Uint8Array(6),view=a.subarray(2,5);const result=new TextEncoder().encodeInto('abcd',view);return[result,Array.from(a)];});
 return out;
})()
;return Object.fromEntries(Object.entries(all).filter(([key])=>new RegExp("^encoder\\.").test(key)));})()