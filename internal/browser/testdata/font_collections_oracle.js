(async()=>{
 ...
 let readyValue=null;fonts.ready.then(value=>{readyValue=value===fonts});await new Promise(resolve=>setTimeout(resolve,10));
 result.set=[Object.prototype.toString.call(fonts),fonts=== (typeof document==='object'?document.fonts:self.fonts),fonts.size,fonts.status,fonts.ready===fonts.ready,readyValue,fonts instanceof EventTarget];
 ...
 result.brandRejection=await FontFace.prototype.load.call({}).catch(e=>e.name);
 return result;
})()
