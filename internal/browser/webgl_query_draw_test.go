...
func TestWebGLQueryDrawFrozenChrome(t *testing.T) {
	serialBrowserTest(t)
	source, err := os.ReadFile("testdata/webgl_query_draw_oracle.js")
	if err != nil {
		t.Fatal(err)
	}
	data, err := os.ReadFile("testdata/webgl_query_draw_chrome152.json")
	if err != nil {
		t.Fatal(err)
	}
	var capture struct {
	...
	for _, restored := range []bool{false, true} {
		t.Run(strconv.FormatBool(restored), func(t *testing.T) {
			...
			if d := bootstrapSnapshotDifference("WebGL query draw", capture.Observation, value); d != "" {
				t.Fatal(d)
			}
		})
	}
