...
func TestWebGLResourceOwnershipAndUnsupportedExecution(t *testing.T) {
	historyTestPages(t, func(t *testing.T, p *Page) {
		...
		historyEval(t, p, `(()=>{const a=new OffscreenCanvas(1,1).getContext('webgl'),b=new OffscreenCanvas(1,1).getContext('webgl');const buffer=a.createBuffer();b.bindBuffer(b.ARRAY_BUFFER,buffer);if(b.getError()!==b.INVALID_OPERATION||b.getParameter(b.ARRAY_BUFFER_BINDING)!==null)throw Error('foreign ownership');const shader=a.createShader(a.VERTEX_SHADER);a.shaderSource(shader,'void main(){gl_Position=vec4(0.0);}');if(!a.getShaderSource(shader).includes('gl_Position'))throw Error('source missing');let unsupported=0;for(const f of [()=>a.compileShader(shader),()=>a.drawArrays(a.TRIANGLES,0,3)])try{f()}catch(e){if(e.name==='NotSupportedError')unsupported++}return unsupported===2&&a.getShaderParameter(shader,a.COMPILE_STATUS)===false})()`, true)
	})
}
