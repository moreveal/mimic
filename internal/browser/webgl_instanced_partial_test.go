...
func TestWebGLInstancedPartialFrozenChrome(t *testing.T) {
	...
	data, err := os.ReadFile("testdata/webgl_instanced_partial_chrome152.json")
	if err != nil {
		t.Fatal(err)
	}
	var capture struct {
	...
	for _, restored := range []bool{false, true} {
		t.Run(strconv.FormatBool(restored), func(t *testing.T) {
			...
			if d := bootstrapSnapshotDifference("WebGL instanced_partial", capture.Observation, value); d != "" {
				t.Fatal(d)
			}
		})
	}
