...
func TestWebGLCompressedSubFrozenChrome(t *testing.T) {
	source, err := os.ReadFile("testdata/webgl_compressed_sub_oracle.js")
	if err != nil {
		t.Fatal(err)
	}
	data, err := os.ReadFile("testdata/webgl_compressed_sub_chrome152.json")
	if err != nil {
		t.Fatal(err)
	}
	var capture struct {
	...
	for _, restored := range []bool{false, true} {
		t.Run(strconv.FormatBool(restored), func(t *testing.T) {
			...
			if d := bootstrapSnapshotDifference("WebGL compressed_sub", capture.Observation, value); d != "" {
				t.Fatal(d)
			}
		})
	}
