...
func TestWebGLInstancedErrorsFrozenChrome(t *testing.T) {
	serialBrowserTest(t)
	source, err := os.ReadFile("testdata/webgl_instanced_errors_oracle.js")
	if err != nil {
		t.Fatal(err)
	}
	data, err := os.ReadFile("testdata/webgl_instanced_errors_chrome152.json")
	if err != nil {
		t.Fatal(err)
	}
	var capture struct {
	...
	for _, restored := range []bool{false, true} {
		t.Run(strconv.FormatBool(restored), func(t *testing.T) {
			...
			if d := bootstrapSnapshotDifference("WebGL instanced_errors", capture.Observation, value); d != "" {
				t.Fatal(d)
			}
		})
	}
